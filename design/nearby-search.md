...
既定4並列（変更範囲1〜8、全国CIは8）、一時的な通信障害は最大8回の試行と指数バックオフ、元ファイルは24時間キャッシュ。全国生成のCIでは--discard-source-cacheを指定し、処理済みPBFを削除してディスク使用を抑える。公式目録に載ったタイルの取得失敗は生成全体を失敗させる。

生成は一時ディレクトリ→不変の版ディレクトリ→manifestの順に置換する。失敗時は既存のmanifestを維持する。同じ版IDの上書きは禁止。検証CLIで全索引先・座標・IDの重複・統計・900MBのデータ予算を確認する。

Pages用に既存の行政区域タイルと新しい版を一つのartifactにまとめる。共通公開ジョブが国外データと合わせて検証し、Actionsで配置する。地域生成は検証artifactのみで公開不可。全国生成のpublish指定、四半期schedule、生成処理の変更時のpushをdefault branchから実行した場合だけ公開用artifactを作成する。GitHub Pagesの設定をGitHub Actionsにする必要がある。
...
近傍検索は標準fetchで取得した解析済みデータをLRUで共有する。国内行政界PBFの24時間キャッシュとは独立して管理する。128エントリ、受信テキスト換算16MiBを上限にする（JavaScriptヒープ使用量の上限ではない）。同時取得4件、待機キュー1024件、各取得は本文受信完了まで15秒・受信バイト数16MiBを上限とする。失敗はキャッシュしない。manifest60秒、版付きタイル24時間。URL全体がキーなので別の配信先・版が混ざらない。
