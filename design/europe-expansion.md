...
検索コードだけを変更した場合は、`build` ワークフローの手動実行で `preview_source_run` を指定すると、
その実行の全65地域の生成済み成果物を最新コードで検証できる。ファイル検証・統合カタログ・実検索を再確認し、
プレビュー成果物を作成する。この経路からは公開しない。
...
- 複数の抽出が重なる場合、自動選択では国を確定できる抽出を優先する。
  Belfastではアイルランド抽出に英国の国境がなく、以前は英国の抽出を試さず国コードnullを返していた。
  地域の明示指定と、国を確定できる抽出がない場合の部分的な結果は保持する。
...
全65地域の生成・国別検証が成功した
[全国再生成](https://github.com/sentium/open-reverse-geocoder/actions/runs/34057296201)の
検証対象OSMデータは1,959,332,005 bytes。国内の検索データ16,755,771 bytesを加えると
1,976,087,776 bytesとなる。この実行の統合段階でBelfastの自動選択の問題を発見し、上記の修正を行った。
保存時にはさらにLICENSE・地域メタデータ・補完履歴等が加わる。

修正後の[全地域再検証](https://github.com/sentium/open-reverse-geocoder/actions/runs/34061528030)では、
全65地域のファイル検証と統合カタログによる検索が成功した。
行政地名107地点を地域の明示指定・自動選択の両方で、近傍施設87地点を明示指定で確認した
（計194サンプル、301回の逆ジオコーダー呼び出し）。
成果物の `validation.json` と `verification.json` に容量・地域・検索結果を保存している。
