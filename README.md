...
ブラジル・メキシコと、欧州全域を目標に小国・島嶼部・周辺国を含む55抽出地域も生成対象に追加しています。範囲と検証状況は
...
全対象のデータはPagesの容量上限を超えるため、OSMを別の静的ホストへ置けます。
カタログの地域ごとの `dataUrl` を使い、検索時は必要な索引と周辺タイルのみ取得します。
[R2配信の設定・公開手順](design/r2-publication.md)を参照してください。
